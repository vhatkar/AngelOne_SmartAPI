...
class LiveTrader:
    ...
    def _action_force_sell_ce(self):
        """Force SELL CE hedge at next level (price-neutral)"""
        leg = self.straddle_manager.ce_leg
        if not leg:
            print("❌ CE leg not found")
            return
        ...
        print(f"\n⚠️ CONFIRM: Force SELL CE hedge at Level {next_level}?")
        print(f"   Current Loss: {leg.current_loss_pct:.1f}%")
        print(f"   Level threshold: {config.PROGRESSIVE_HEDGING_LEVELS[next_level-1]:.1f}%")
        print(f"   ⚠️ Will SELL opposing strike at MARKET PRICE")
        confirm = input("   Type 'YES' to confirm: ").strip().upper()
        ...
        self._execute_force_sell_hedge(leg, 'CE', next_level)
    
    def _action_force_sell_pe(self):
        """Force SELL PE hedge at next level (price-neutral)"""
        leg = self.straddle_manager.pe_leg
        if not leg:
            print("❌ PE leg not found")
            return
        ...
        print(f"\n⚠️ CONFIRM: Force SELL PE hedge at Level {next_level}?")
        print(f"   Current Loss: {leg.current_loss_pct:.1f}%")
        print(f"   Level threshold: {config.PROGRESSIVE_HEDGING_LEVELS[next_level-1]:.1f}%")
        print(f"   ⚠️ Will SELL opposing strike at MARKET PRICE")
        confirm = input("   Type 'YES' to confirm: ").strip().upper()
        ...
        self._execute_force_sell_hedge(leg, 'PE', next_level)
    ...
    def _action_force_exit_ce(self):
        ...
        print(f"   ⚠️ Will BUY at MARKET PRICE")
        confirm = input("   Type 'YES' to confirm: ").strip().upper()
    ...
    def _execute_force_sell_hedge(self, leg, leg_type: str, level: int):
        """Execute force SELL hedge with CORRECTED price-neutral logic"""
        self.is_executing_trade = True
        
        try:
            print(f"\n{'='*80}")
            print(f"🛡️ FORCE SELLING {leg_type} HEDGE LEVEL {level}")
            print(f"{'='*80}")
            ...
            hedge_strike, target_premium = hm._calculate_price_neutral_strike(
            ...
            leg.sell_hedge(hedge_symbol, hedge_security_id, hedge_strike, hedge_premium, level)
            
            if self.excel_logger:
                self.excel_logger.log_manual_intervention(
                    intervention_type='FORCESELL',
                    leg_type=leg_type,
                    level=level,
                    time=config.get_current_ist_time(),
                    notes=f"Force sell via menu at {leg.current_loss_pct:.1f}% loss"
                )
            ...
            print(f"✅ {leg_type} hedge Level {level} SOLD!")
            print(f"{'='*80}")
