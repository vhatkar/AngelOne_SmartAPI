"""
Individual Leg Class for CE/PE tracking - PURE PRICE-NEUTRAL HEDGING
NO BUFFER/NO TRAILING - Hold hedges until Level 3

100% CORE LOGIC COMPLIANT + PURE SELL HEDGE STRATEGY
🔥 HYBRID: Updated for 2-level price-neutral + Level 3 hard stop
✅ CORRECTED: SELL hedge P&L calculations
✅ ADDED: Missing methods for price neutrality
...
class Leg:
    ...
    def should_sell_hedge(self) -> bool:
        """
        ✅ TRIGGER FIX: Check if hedge should be sold
        🔥 FIXED: Prevents immediate re-sell after manual exit
        🔥 CRITICAL FIX: Added loading/unloading mechanism
        🔥 UPGRADE FIX: Auto-reload ONLY during reversals, not during upgrades
        """
        if self.hedge_active:
            return False
        
        # 🔥 NEW: Prevent immediate re-sell after manual exit (1 minute cooldown)
        ...
        # ✅ CRITICAL FIX: REMOVED auto-reload logic from here
        # Auto-reload is now ONLY handled by:
        # 1. hedge_manager.process_leg_hedging() - for safe conditions
        # 2. leg.close_hedge() - during reversal exits
        # This prevents state corruption during upgrade transitions
    ...
    def sell_hedge(self, hedge_symbol: str, hedge_security_id: str,
        ...
        """PURE SELL HEDGE - Hold until Level 3"""
        ...
        print(f"\n{'='*60}")
        print(f"✅ {self.name} L{level} SELL HEDGE")
        print(f"{'='*60}")
    ...
    def is_level_3_triggered(self) -> bool:
        ...
        """
        # 🔥 CRITICAL FIX: Removed abs() - only trigger on actual loss >= 60%
    ...
    def get_pnl(self) -> float:
        ...
        # Current hedge P&L (if hedge is active) - ✅ CORRECTED for SELL
    ...
    def get_pnl_breakdown(self) -> Dict:
        ...
        # ✅ CORRECTED: SELL hedge P&L calculation
        current_hedge_pnl = 0.0
        if self.hedge_active and self.hedge_current_premium:
            current_hedge_pnl = (self.hedge_entry_premium - self.hedge_current_premium) * self.lot_size
    ...
    def sync_manual_hedge_exit(self, exited_level: int):
        ...
        if self.hedge_active:
            ...
            if self.hedge_entry_premium and self.hedge_current_premium:
                hedge_pnl = (self.hedge_entry_premium - self.hedge_current_premium) * self.lot_size
