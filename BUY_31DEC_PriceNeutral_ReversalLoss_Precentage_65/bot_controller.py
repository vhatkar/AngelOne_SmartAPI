...
class BotController:
    ...
    def _show_menu(self):
        ...
        while True:
            ...
            print("   3 - Force BUY CE hedge (Manual CE hedge entry)")
            print("   4 - Force BUY PE hedge (Manual PE hedge entry)")
            ...
            try:
                ...
                if choice == 'P':
                    ...
                    self._handle_force_buy_ce()
                elif choice == '4':
                    self._handle_force_buy_pe()
                elif choice == '5':
    ...
    def _handle_force_buy_ce(self):
        """Force BUY CE hedge"""
        if self.trader.straddle_manager.straddle_active:
            self.trader._action_force_buy_ce()
        else:
            print("❌ No active straddle")
        # 🔥 FIX 2: No input wait - returns to menu immediately
    
    def _handle_force_buy_pe(self):
        """Force BUY PE hedge"""
        if self.trader.straddle_manager.straddle_active:
            self.trader._action_force_buy_pe()
        else:
            print("❌ No active straddle")
        # 🔥 FIX 2: No input wait - returns to menu immediately
