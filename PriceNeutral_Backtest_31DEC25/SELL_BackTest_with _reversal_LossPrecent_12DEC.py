...
- When losing leg reaches configured % from original price:
  * Level 1: 20% loss → Sell hedge on profit side to neutralize
  * Level 2: 40% loss → Square off L1 hedge, sell new hedge to neutralize
...
- Hedges sold (not bought) to collect premium and maintain price neutrality

Version: 1.0_PRICE_NEUTRAL_FIXED
BUG FIX: Fixed display bug in _enter_hedge_at_level() - hedge now correctly shown on profit side
BUG FIX: Added loading/unloading logic for L1 and L2 triggers
✅ BUG FIX: Removed auto-reload from _manage_leg_hedge() to match live trading
✅ BUG FIX: Added reload logic only in _check_hedge_exit() on reversal exits
✅ FEATURE: Added HEDGE_REVERSAL_EXIT_PCT parameter for adjustable hedge exit thresholds
"""
...
class BacktestConfig:
    ...
    # Strategy Parameters - Price Neutral Hedging
...
class HistoricalDataManager:
    ...
    def find_hedge_strike_by_premium(self, target_premium, option_type, timestamp, current_strike):
        """Find strike with premium closest to target on profit side"""
        if target_premium <= 0:
            return None, None
        ...
        # For CE: search below current (profit side when market up)
        # For PE: search above current (profit side when market down)
        if option_type == 'PE':
            # Search below current strike (OTM puts)
            start_strike = base_strike - (search_range * config.STRIKE_INTERVAL)
            end_strike = current_strike - config.STRIKE_INTERVAL
        else:  # CE
            # Search above current strike (OTM calls)
...
class Leg:
    ...
    def get_hedge_pnl(self):
        ...
        if not self.hedge_active or not self.hedge_current_premium:
            return 0
        # We SOLD the hedge, so profit when it goes down
        return (self.hedge_entry_premium - self.hedge_current_premium) * config.LOT_SIZE
...
class StraddleManager:
    ...
    def log_entry(self, timestamp, session_number):
        """Log straddle entry (called from backtester after session number is set)"""
        # Log transaction
...
# ============================================================================
# HEDGE MANAGER - NEW PRICE NEUTRAL LOGIC
# ============================================================================

class HedgeManager:
    """Manages price-neutral hedging strategy"""
    ...
    def _manage_leg_hedge(self, timestamp, losing_leg, profit_leg, leg_type):
        ...
        # ✅ CRITICAL FIX: AUTO-RELOAD LOGIC COMPLETELY REMOVED FROM HERE
        # This matches live trading - auto-reload ONLY happens on reversal exits
        ...
        if losing_leg.hedge_active:
            ...
            if should_upgrade:
                ...
                # Reset hedge state but keep level for tracking
    ...
    def _check_hedge_exit(self, timestamp, losing_leg, current_loss_pct, leg_type):
        """Check if hedge should be exited (losing leg returned to trigger price)"""
        
        if losing_leg.hedge_level == 1:
            # Level 1: exit when loss retraces by configured % from L1 entry point
            exit_trigger = config.LEVEL1_TRIGGER_PCT - config.HEDGE_REVERSAL_EXIT_PCT
        elif losing_leg.hedge_level == 2:
            # Level 2: exit when loss retraces by configured % from L2 entry point
        ...
        if current_loss_pct <= exit_trigger:
            ...
            # ✅ CRITICAL FIX: RELOAD LOGIC ADDED HERE (ONLY ON REVERSAL EXIT)
            # This matches live trading - reload happens ONLY when hedge exits due to reversal
            if losing_leg.hedge_level == 1:
                losing_leg.l1_loaded = True  # L1 reloaded after L1 exit
            elif losing_leg.hedge_level == 2:
                losing_leg.l2_loaded = True  # L2 reloaded after L2 exit
            ...
            losing_leg.hedge_entry_premium = None
            losing_leg.hedge_current_premium = None
            # Keep hedge_level for re-trigger detection
    ...
    def _check_hedge_entry(self, timestamp, losing_leg, profit_leg, current_loss_pct, leg_type):
        ...
        if current_loss_pct >= config.LEVEL3_TRIGGER_PCT:
            # Level 3 triggers complete exit (handled in backtester)
            return
        ...
        if candidate_level == 1:
            if not losing_leg.l1_loaded:
                return  # L1 not loaded, cannot trigger ❌
        elif candidate_level == 2:
            if not losing_leg.l2_loaded:
                return  # L2 not loaded, cannot trigger ❌
    ...
    def _enter_hedge_at_level(self, timestamp, losing_leg, profit_leg, current_loss_pct, leg_type, target_level):
        ...
        # Find hedge strike on PROFIT side with premium ≈ difference
        profit_option_type = profit_leg.option_type
        hedge_strike, hedge_symbol = self.data_manager.find_hedge_strike_by_premium(
            difference, profit_option_type, timestamp, profit_leg.strike
        )
        ...
        # SELL the hedge (we collect premium)
        ...
        losing_leg.total_hedges_sold += 1
        
        # ✅ Unload the level that just triggered
        if target_level == 1:
            losing_leg.l1_loaded = False  # L1 now unloaded
        elif target_level == 2:
            losing_leg.l2_loaded = False  # L2 now unloaded
        ...
        self.tx_logger.log(timestamp, 'HEDGE_ENTRY', {
            ...
            'hedge_type': profit_option_type,
            'difference': difference
        })
        
        # CORRECTED: Calculate neutral prices for display
        # Hedge is sold on PROFIT side, so add it there
        if leg_type == 'CE':  # CE losing, hedge is PE
            ce_total = losing_leg.current_premium  # CE raw only
            pe_total = profit_leg.current_premium + hedge_price  # PE + hedge
        else:  # PE losing, hedge is CE
            ce_total = profit_leg.current_premium + hedge_price  # CE + hedge
            pe_total = losing_leg.current_premium  # PE raw only
        
        print(f"     [{timestamp.strftime('%H:%M:%S')}] [HEDGE] {leg_type} L{target_level}: SELL {profit_option_type} {int(hedge_strike)} @ Rs.{hedge_price:.2f}")
        print(f"             Entry Loss: {current_loss_pct:.1f}%")
        print(f"             Price Neutral: CE={ce_total:.2f}, PE={pe_total:.2f}")
...
class PriceNeutralBacktester:
    """Main backtester for price-neutral strategy"""
    ...
    def run_backtest(self, start_date, end_date):
        """Run the backtest"""
        print("\n" + "="*80)
        print("[START] PRICE NEUTRAL PROGRESSIVE HEDGING BACKTEST")
        print("="*80)
            ...
            if not self.straddle_manager.straddle_active:
                if config.ENTRY_WINDOW_START <= trade_time <= config.ENTRY_WINDOW_END:
                    ...
                    if self.straddle_manager.enter_straddle(timestamp):
                        # Log entry with correct session number
    ...
    def exit_straddle(self, timestamp, reason):
        ...
        session_data = {
            ...
            'total_pnl': total_pnl,
            'exit_reason': reason,
            'ce_hedges': ce.total_hedges_sold,
            'pe_hedges': pe.total_hedges_sold,
        ...
        self.daily_stats[trade_date]['ce_hedges'] += ce.total_hedges_sold
        self.daily_stats[trade_date]['pe_hedges'] += pe.total_hedges_sold
    ...
    def generate_results(self):
        ...
        for date in sorted(self.daily_stats.keys()):
            ...
            total_h = ce_h + pe_h
            print(f"📅 {date} | Sessions: {stats['sessions']:2d} | P&L: {pnl_sign}Rs.{pnl:,.2f} | Hedges: {total_h:2d} (CE:{ce_h}, PE:{pe_h})")
        print("="*80)
        print(f"✅ Total P&L ({min(self.daily_stats.keys())} → {max(self.daily_stats.keys())}): Rs.{total_pnl:,.2f}")
        print(f"📊 Total Sessions: {total_sessions} | Total Hedges: {total_ce_hedges + total_pe_hedges} (CE:{total_ce_hedges}, PE:{total_pe_hedges})")
        if total_sessions > 0:
        ...
        print("\n" + "="*80)
        print("[RESULTS] BACKTEST SUMMARY - PRICE NEUTRAL STRATEGY")
        print("="*80)
        ...
        print(f"Total Hedges: CE={total_ce_hedges}, PE={total_pe_hedges}")
...
if __name__ == "__main__":
    print("\n🙏 Ganapati Bappa Morya! 🙏")
    print("Starting Price-Neutral Progressive Hedging Backtest...")
    ...
    try:
        ...
        if results is not None:
            output_file = f"PriceNeutral_Backtest_{START_DATE}_to_{END_DATE}.xlsx"
